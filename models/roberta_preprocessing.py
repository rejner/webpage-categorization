...
class NERTokenizerForRoBERTa(keras.layers.Layer):
    """
    ...
    Tokens must be preprocessed before feeding into RoBERTa model.
    Also, the labels must be adjusted to the new tokens after preprocessing.
    """
    def __init__(self, seq_length=256, **kwargs):
        super(NERTokenizerForRoBERTa, self).__init__(**kwargs)
        self.preprocess = hub.load('https://tfhub.dev/jeongukjae/roberta_en_cased_preprocess/1')
        ...
        self.vocab_file = 'models/vocab_roberta.json'
    ...
    def call(self, inputs):
        ...
        def loop_body(i, labels):
            ...
            # extend the first dimension of word_labels to match the number of tokens
            word_label = tf.tile(word_labels[i], [shape[0]])
        ...
        tokenized_inputs = tf.RaggedTensor.from_tensor(tokenized_inputs, padding=0)
        encoder_inputs = self.bert_pack_inputs([tokenized_inputs])
        # input_type_ids, input_mask, input_word_ids = encoder_inputs.values
...
def run_test_mode(ds):
    ...
    for i, example in enumerate(ds):
        ...
        example_tokens = ['<s>'] + example_tokens + ['</s>']
